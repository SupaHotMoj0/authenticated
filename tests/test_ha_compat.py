...
def test_config_flow_file_exists():
    ...
    assert os.path.isfile(os.path.join(REPO_ROOT, "config_flow.py"))


def test_strings_json_exists():
    """strings.json must exist for config flow UI."""
    assert os.path.isfile(os.path.join(REPO_ROOT, "strings.json"))
...
def test_init_has_entry_lifecycle():
    ...
    with open(os.path.join(REPO_ROOT, "__init__.py")) as f:
        source = f.read()
...
def test_providers_has_async_method():
    ...
    with open(os.path.join(REPO_ROOT, "providers.py")) as f:
        source = f.read()
